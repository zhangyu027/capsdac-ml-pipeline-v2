...
def make_candidate_models(random_state: int = 2026) -> Dict[str, tuple[Any, Dict[str, list[Any]]]]:
    """Return interview-friendly baseline and tree models.

    XGBoost/LightGBM are intentionally optional in this public package to keep the
    repo lightweight. The gradient boosting model is used as the default
    production-like booster because it is available from scikit-learn.
    """
    return {
        ...
        "random_forest": (
            ...
            {"n_estimators": [50, 120], "max_depth": [8, 12], "min_samples_leaf": [1, 2]},
        ),
        "gradient_boosting": (
            GradientBoostingRegressor(random_state=random_state),
            {"n_estimators": [50, 120], "learning_rate": [0.03, 0.08], "max_depth": [2, 3]},
        ),
    }


def expanding_window_splits(feature_df: pd.DataFrame, min_train_months: int = 18, horizon_months: int = 1) -> List[Dict[str, Any]]:
    months = sorted(pd.to_datetime(feature_df["MonthDate"]).drop_duplicates())
    splits: List[Dict[str, Any]] = []
    for test_start_idx in range(min_train_months, len(months) - horizon_months + 1):
        train_months = months[:test_start_idx]
        test_months = months[test_start_idx : test_start_idx + horizon_months]
        splits.append(
            {
...
def evaluate_model_class(feature_df: pd.DataFrame, name: str, estimator: Any, params: Dict[str, Any], splits: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for fold_num, split in enumerate(splits, start=1):
        ...
        model.set_params(**params) if hasattr(model, "set_params") else None
        train = feature_df.loc[split["train_mask"]]
        test = feature_df.loc[split["test_mask"]]
        model.fit(train[FEATURES], train["EnrollmentCount"])
        pred = model.predict(test[FEATURES])
        m = regression_metrics(test["EnrollmentCount"], pred)
...
def run_model_selection(feature_df: pd.DataFrame, max_param_sets_per_model: int = 8) -> Tuple[Any, Dict[str, Any], pd.DataFrame]:
    ...
    best_model.fit(feature_df[FEATURES], feature_df["EnrollmentCount"])
    ...
    return best_model, {"selected_model": best_record, "all_results": results}, leaderboard
